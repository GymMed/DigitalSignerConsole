...
namespace FirmaXadesNetCoreUpdated.Validation
{
    class XadesValidator
    {
        ...
        public ValidationResult Validate(SignatureDocument sigDocument)
        {
            ...
            catch (Exception ex)
            {
                ...
                result.Message = "Parašo patikrinimas nepavyko";// "La verificación de la firma no ha sido satisfactoria";
            ...
            if (sigDocument.XadesSignature.UnsignedProperties.UnsignedSignatureProperties.SignatureTimeStampCollection.Count > 0)
            {
                ...
                FirmaXadesNetCore.Crypto.DigestMethod tsDigestMethod = FirmaXadesNetCore.Crypto.DigestMethod.GetByOid(token.TimeStampInfo.HashAlgorithm.ObjectID.Id);
                ...
                {
                    result.IsValid = false;
                    result.Message = "Laiko žymos piršto atspaude neatitinka apskaičiavimo";// "La huella del sello de tiempo no se corresponde con la calculada";
            ...
            result.IsValid = true;
            result.Message = "Parašo patvirtinimas sėkmingas!";// "Verificación de la firma satisfactoria";
